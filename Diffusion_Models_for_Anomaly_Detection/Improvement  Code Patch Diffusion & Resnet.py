# -*- coding: utf-8 -*-
"""
BRATS Diffusion Anomaly Detection – v4 (CPU-only, Patch-based + ResNet classifier)

Changes vs v3:
4) Patch-based diffusion sampling:
   - Sliding-window reconstruction over patches (default 128x128, stride 64)
   - Improves detection of small and subtle tumors
   - Reduces over-smoothing from full-image diffusion

5) Stronger classifier backbone (ResNet18):
   - Time-conditioned ResNet classifier for better guidance
   - Produces cleaner "healthy" reconstructions
   - Improves anomaly localization
...
- Anomaly map from SSIM/L2 + Gaussian smoothing on FLAIR
- Evaluation: slice-wise Dice (Otsu) + pixel-wise AUROC + slice-wise AUROC
- Tkinter GUI for:
    * Train DDPM
    * Train classifier
    * Run anomaly detection on a single FLAIR volume (mid-slice)
"""
...
PATCH_SIZE_DEFAULT = 128            # for patch-based diffusion
PATCH_STRIDE_DEFAULT = 64


# =========================
# UNet + time embedding for DDPM
# =========================
...
# =========================
# Time-conditioned ResNet classifier
# =========================

class TimeCondResNetClassifier(nn.Module):
    """
    Stronger classifier backbone using ResNet-18, with time embedding.

    - First conv changed to 4-channel input
    - Time embedding injected after first BN layer
    """

    def __init__(self, in_channels=4, time_dim=256, num_classes=2):
        super().__init__()
        self.time_mlp = TimeEmbedding(time_dim)
        self.time_to_ch = nn.Linear(time_dim, 64)  # ResNet conv1 out_channels = 64
    ...
    def forward(self, x, t):
        ...
        # ResNet stem with time injection
        x = self.conv1(x)
        x = self.bn1(x)
        x = x + t_ch
        x = self.relu(x)
...
# =========================
# Training: ResNet classifier on noisy x_t
# =========================
...
def train_classifier(
    ...
    clf = TimeCondResNetClassifier(in_channels=4, time_dim=256, num_classes=2)
    clf.to(DEVICE)
    ...
    for epoch in range(epochs):
        clf.train()
        running = 0.0
        pbar = tqdm(loader, desc=f"Classifier (ResNet) Epoch {epoch + 1}/{epochs}")
        for x, y, _, _ in pbar:
            x = x.to(DEVICE)
            y = y.to(DEVICE)
            b = x.size(0)
        ...
        ckpt_path = os.path.join(out_dir, f"classifier_resnet_brats4ch_epoch{epoch + 1}.pt")
        torch.save(
            {
...
# =========================
# Patch-based diffusion reconstruction (sliding window)
# =========================

def patch_based_guided_reconstruction(
    ddpm: DDPM,
    clf: TimeCondResNetClassifier,
    ...
    stride: int = PATCH_STRIDE_DEFAULT,
):
    """
    Sliding-window patch-based guided reconstruction.

    x0_full: (1, C, H, W)
    Returns: x_healthy_full with same shape.
    """
    assert x0_full.dim() == 4 and x0_full.size(0) == 1, "Expect (1,C,H,W)"
    _, C, H, W = x0_full.shape

    out = torch.zeros_like(x0_full)
    weight = torch.zeros_like(x0_full)
    ...
    for y0 in ys:
        for x0 in xs:
            patch = x0_full[:, :, y0:y0 + patch_size, x0:x0 + patch_size]
            x_healthy_patch = guided_translate_to_healthy(
                ddpm, clf, patch, steps=steps, guidance_scale=guidance_scale
            )
            out[:, :, y0:y0 + patch_size, x0:x0 + patch_size] += x_healthy_patch
...
# =========================
# Anomaly mapping: SSIM/L2 + Gaussian smoothing
# =========================
...
# =========================
# Single-case anomaly (mid-slice, GUI)
# =========================
...
def run_anomaly_single(
    ...
    print("Loaded ResNet classifier:", clf_ckpt)
    ...
    # --- patch-based guided reconstruction ---
    x_healthy = patch_based_guided_reconstruction(
        ddpm, clf, x0,
        steps=steps,
        guidance_scale=guidance_scale,
    ...
    axs[1].set_title("Healthy reconstruction (patch-based)")
    ...
    out_png = os.path.join(out_dir, f"{base_name}_anomaly_flair_guided_v4.png")
    plt.savefig(out_png, dpi=150)
    plt.show()

    out_csv = os.path.join(out_dir, f"{base_name}_anomaly_flair_guided_v4.csv")
...
# =========================
# Evaluation on BRATS (patch-based + ResNet)
# =========================
...
def evaluate_brats(
    ...
    print(f"Evaluating on {len(seg_files)} subjects (v4 patch-based + ResNet)...")
    ...
    metrics_path = os.path.join(out_dir, "metrics_v4.txt")
    with open(metrics_path, "w") as f:
        f.write(f"Mean Dice (Otsu, slice-wise, v4): {mean_dice:.4f}\n")
        f.write(f"Global AUROC (pixel-level, v4): {auc_pixels:.4f}\n")
        f.write(f"Slice-wise AUROC (mean anomaly vs tumor presence): {auc_slices:.4f}\n")

    print("===== Evaluation finished (v4) =====")
    ...
    slice_csv_path = os.path.join(out_dir, "slice_metrics_v4.csv")
    ...
    summary_csv_path = os.path.join(out_dir, "summary_metrics_v4.csv")
    df_summary.to_csv(summary_csv_path, index=False)
    print("Saved summary metrics CSV:", summary_csv_path)


# =========================
# Tkinter GUI (kept intact, now v4)
# =========================

class AnomalyGUI:
    def __init__(self, root):
        self.root = root
        root.title("BRATS DDPM – v4 (Patch-based + ResNet classifier, CPU)")
        ...
        tk.Button(root, text="Train classifier (ResNet)",
        ...
        tk.Button(root, text="Run anomaly detection (v4 patch-based)",
...
def main():
    ...
    if args.mode == "train_ddpm":
        ...
        # Reuse v3 sweep but pointing to v4 eval (patch-based)
        settings = [
            (20, 3.0),
            (20, 5.0),
            (40, 3.0),
        ...
        for steps, s in settings:
            ...
            out_dir = os.path.join("eval_sweep_v4", tag)
            evaluate_brats(
                data_root=args.data_root,
                ddpm_ckpt=args.ddpm_ckpt,
                clf_ckpt=args.clf_ckpt,
