# 2D Generative Adversarial Network (GAN) from Scratch
...
- **Gaussian** (`gaussian`): Single 2D Gaussian distribution centered at `[2, 2]` with a variance scale of `1.2`.
...
- **Mode Collapse & Recovery**: Continued training demonstrated that the Generator could dynamically escape partial mode collapse and cover additional modes.
...
Start training with the default settings (`mixture4` distribution):

```bash
python main.py
```

To switch datasets, update the `mode` parameter inside `main.py`:

```python
mode = "gaussian"   # Single mode
...
- `frames/`: Contains PNG snapshots of real vs. generated distribution alignment saved every *N* steps (e.g., `frame_00005.png`).
...
- Explore alternate loss formulations (e.g., Least Squares GAN).
